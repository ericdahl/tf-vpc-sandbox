...
resource "aws_lb" "nlb" {
  ...
  subnets = [
    "${aws_subnet.10_1_0_0_private1.id}",
    "${aws_subnet.10_1_0_0_private2.id}",
    "${aws_subnet.10_1_0_0_private3.id}"
  ]
}
...
resource "aws_lb_target_group_attachment" "nlb" {
  target_group_arn = "${aws_lb_target_group.nlb_tg.arn}"
  target_id = "${aws_instance.10_1_0_0_jumphost.id}"
  port = 80
}

resource "aws_instance" "10_1_0_0_webserver" {
  ami           = "${data.aws_ami.freebsd_11.image_id}"
  instance_type = "t2.small"
  subnet_id     = "${aws_subnet.10_1_0_0_private1.id}"
...
resource "aws_flow_log" "10_1_0_0" {
